...
static QRinput_List *QRinput_List_newEntry(QRencodeMode mode, int size, const unsigned char *data)
{
	...
	entry = (QRinput_List *)Malloc(sizeof(QRinput_List), eHeapTemp);
	if(entry == NULL) return NULL;

	entry->mode = mode;
	entry->size = size;
	entry->data = (unsigned char *)Malloc(size, eHeapTemp);
	if(entry->data == NULL) {
...
{
	if(entry != NULL) {
		Free(entry->data, entry->size, eHeapTemp);
...
static QRinput_List *QRinput_List_dup(QRinput_List *entry)
{
	...
	n = (QRinput_List *)Malloc(sizeof(QRinput_List), eHeapTemp);
	if(n == NULL) return NULL;

	n->mode = entry->mode;
	n->size = entry->size;
	n->data = (unsigned char *)Malloc(n->size, eHeapTemp);
	if(n->data == NULL) {
		Free((U8*)n, sizeof(QRinput_List), eHeapTemp);
		return NULL;
	}
...
QRinput *QRinput_new2(int version, QRecLevel level)
{
	...
	input = (QRinput *)Malloc(sizeof(QRinput), eHeapTemp);
	memset(input, 0, sizeof(QRinput));
...
void QRinput_free(QRinput *input)
{
	...
	if(input != NULL) {
		...
		Free((U8*)input, sizeof(QRinput), eHeapTemp);
	}
}
...
static int QRinput_encodeBitStream(QRinput_List *entry, int version)
{
	...
	length = BitStream_size(entry->bstream);
	return length;
...
static int QRinput_appendPaddingBit(BitStream *bstream, QRinput *input)
{
	...
	if(padlen > 0) {
		padbuf = (unsigned char *)Malloc(padlen, eHeapTemp);
		if(padbuf == NULL) {
			ret = -1;
			goto DONE;
		}
...
static QRinput_InputList *QRinput_InputList_newEntry(QRinput *input)
{
	...
	entry = (QRinput_InputList *)Malloc(sizeof(QRinput_InputList), eHeapTemp);
	if(entry == NULL) return NULL;
...
static void QRinput_InputList_freeEntry(QRinput_InputList *entry)
{
	if(entry != NULL) {
		...
		Free((U8*)entry, sizeof(QRinput_InputList), eHeapTemp);
	}
}
...
QRinput_Struct *QRinput_Struct_new(void)
{
	...
	s = (QRinput_Struct *)Malloc(sizeof(QRinput_Struct), eHeapTemp);
...
void QRinput_Struct_free(QRinput_Struct *s)
{
	...
	if(s != NULL) {
		...
		Free((U8*)s, sizeof(QRinput_Struct), eHeapTemp);
	}
}
...
static int QRinput_List_shrinkEntry(QRinput_List *entry, int bytes)
{
	...
	data = (unsigned char *)Malloc(bytes, eHeapTemp);
...
int QRinput_Struct_insertStructuredAppendHeaders(QRinput_Struct *s)
{
	...
	list = s->head;
	while(list != NULL) {
		if(QRinput_insertStructuredAppendHeader(list->input, num, i, (unsigned char)s->parity))
			return -1;
		i++;
		list = list->next;
	}
