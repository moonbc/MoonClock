...
static int RSblock_init(RSblock *blocks, int spec[5], unsigned char *data, unsigned char *ecc)
{
	...
	{
		free_rs_cache();//Memory Leak 때문에 추가(수정)
		return 0;
	}
...
__STATIC QRRawCode *QRraw_new(QRinput *input)
{
	...
	raw = (QRRawCode *)Malloc(sizeof(QRRawCode), eHeapTemp);
	if(raw == NULL) return NULL;

	raw->datacode = QRinput_getByteStream(input);
	if(raw->datacode == NULL) {
		Free((U8*)raw, sizeof(QRRawCode), eHeapTemp);
		return NULL;
	}
	...
	raw->ecccode = (unsigned char *)Malloc(raw->eccLength, eHeapTemp);
	if(raw->ecccode == NULL) {
		Free(raw->datacode, raw->dataLength, eHeapTemp);
		Free((U8*)raw, sizeof(QRRawCode), eHeapTemp);
		return NULL;
	}

	raw->blocks = QRspec_rsBlockNum(spec);
	raw->rsblock = (RSblock *)Malloc(sizeof(RSblock) * raw->blocks, eHeapTemp);
	if(raw->rsblock == NULL) {
		QRraw_free(raw);
		return NULL;
	}
...
__STATIC void QRraw_free(QRRawCode *raw)
{
	if(raw != NULL) {
		Free(raw->datacode, raw->dataLength, eHeapTemp);
		Free(raw->ecccode, raw->eccLength, eHeapTemp);
		if(raw->rsblock != NULL) {
			Free((U8*)raw->rsblock, sizeof(RSblock) * raw->blocks, eHeapTemp);
		}
		Free((U8*)raw, sizeof(QRRawCode), eHeapTemp);
	}
}
...
static FrameFiller *FrameFiller_new(int width, unsigned char *frame)
{
	...
	filler = (FrameFiller *)Malloc(sizeof(FrameFiller), eHeapTemp);
...
unsigned char *FrameFiller_fillerTest(int version)
{
	...
	p = FrameFiller_next(filler);
	Free(filler,sizeof(FrameFiller), eHeapTemp);
	if(p != NULL) {
		return NULL;
	}
...
static QRcode *QRcode_new(int version, int width, unsigned char *data)
{
	...
	qrcode = (QRcode *)Malloc(sizeof(QRcode), eHeapTemp);
...
void QRcode_free(QRcode *qrcode)
{
	if(qrcode != NULL) 
	{
		Free(qrcode->data, qrcode->width * qrcode->width, eHeapTemp);
		Free((U8*)qrcode, sizeof(QRcode), eHeapTemp);
	}
}
...
__STATIC QRcode *QRcode_encodeMask(QRinput *input, int mask)
{
	...
	version = raw->version;
	width = QRspec_getWidth(version);
	frame = QRspec_newFrame(version, &ulframeSize);
	...
	if(filler == NULL) {
		QRraw_free(raw);
		Free(frame, ulframeSize, eHeapTemp);
		return NULL;
	}
	...
	Free((U8*)filler, sizeof(FrameFiller), eHeapTemp);
...
static QRcode_List *QRcode_List_newEntry(void)
{
	...
	entry = (QRcode_List *)Malloc(sizeof(QRcode_List), eHeapTemp);
...
static void QRcode_List_freeEntry(QRcode_List *entry)
{
	if(entry != NULL) {
		...
		Free((U8*)entry, sizeof(QRcode_List), eHeapTemp);
	}
}
