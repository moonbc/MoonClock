...
unsigned char *Mask_makeMask(int width, unsigned char *frame, int mask, QRecLevel level)
{
	...
	masked = (unsigned char *)Malloc(width * width, eHeapTemp);
...
unsigned char *Mask_mask(int width, unsigned char *frame, QRecLevel level)
{
	...
	mask = (unsigned char *)Malloc(width * width, eHeapTemp);
...
//		printf("(%d,%d,%d,%d)=%d\n", n1, n2, n3 ,n4, demerit);
		if(demerit < minDemerit) {
			...
			bestMaskNum = i;
			if(bestMask != NULL) {
				Free(bestMask, width * width, eHeapTemp);
			}
			bestMask = (unsigned char *)Malloc(width * width, eHeapTemp);
			if(bestMask == NULL) break;
			memcpy(bestMask, mask, width * width);
		}
	}
	//
	Free(mask, width * width, eHeapTemp);
	return bestMask;
}
