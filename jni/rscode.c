...
//#define NULL ((void *)0)
  ...
  rs = (RS *)Malloc(sizeof(RS), eHeapTemp);
  if(rs == NULL)
    return rs;
  ...
  rs->alpha_to = (data_t *)Malloc(sizeof(data_t)*(rs->nn+1), eHeapTemp);
  if(rs->alpha_to == NULL){
    Free((U8*)rs, sizeof(RS), eHeapTemp);
    rs = NULL;
    return rs;
  }
  rs->index_of = (data_t *)Malloc(sizeof(data_t)*(rs->nn+1), eHeapTemp);
  if(rs->index_of == NULL){
	  Free(rs->alpha_to, sizeof(data_t)*(rs->nn+1), eHeapTemp);
    Free((U8*)rs, sizeof(RS), eHeapTemp);
    rs = NULL;
    return rs;
  }
  ...
  if(sr != 1){
    ...
    Free(rs->alpha_to, sizeof(data_t)*(rs->nn+1), eHeapTemp);
    Free(rs->index_of, sizeof(data_t)*(rs->nn+1), eHeapTemp);
    Free((U8*)rs, sizeof(RS), eHeapTemp);
    rs = NULL;
    return rs;
  }
  ...
  rs->genpoly = (data_t *)Malloc(sizeof(data_t)*(nroots+1), eHeapTemp);
  if(rs->genpoly == NULL){
    Free(rs->alpha_to, sizeof(data_t)*(rs->nn+1), eHeapTemp);
    Free(rs->index_of, sizeof(data_t)*(rs->nn+1), eHeapTemp);
    Free((U8*)rs, sizeof(RS), eHeapTemp);
...
void free_rs_char(RS *rs)
{
	...
	Free(rs->alpha_to, sizeof(data_t)*(rs->nn+1), eHeapTemp);
	Free(rs->index_of, sizeof(data_t)*(rs->nn+1), eHeapTemp);
	Free(rs->genpoly, sizeof(data_t)*(rs->nroots+1), eHeapTemp);
	Free((U8*)rs, sizeof(RS), eHeapTemp);
