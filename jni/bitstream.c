...
BitStream *BitStream_new(void)
{
	...
	bstream = (BitStream *)Malloc(sizeof(BitStream), eHeapTemp);
...
static int BitStream_allocate(BitStream *bstream, int length)
{
	...
	data = (unsigned char *)Malloc(length, eHeapTemp);
	if(data == NULL) {
		return -1;
	}

	if(bstream->data) {
		Free(bstream->data, bstream->length, eHeapTemp);
	}
...
int BitStream_append(BitStream *bstream, BitStream *arg)
{
	...
	data = (unsigned char *)Malloc(bstream->length + arg->length, eHeapTemp);
	if(data == NULL) {
		return -1;
	}
...
unsigned char *BitStream_toByte(BitStream *bstream)
{
	...
	data = (unsigned char *)Malloc((size + 7) / 8, eHeapTemp);
	if(data == NULL) {
		return NULL;
	}
...
void BitStream_free(BitStream *bstream)
{
	if(bstream != NULL) {
		Free(bstream->data, bstream->length, eHeapTemp);
		Free((U8*)bstream, sizeof(BitStream), eHeapTemp);
	}
}
