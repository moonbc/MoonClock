...
char* qr_strdup(const char * _Src);
...
static char *dupAndToUpper(const char *str, QRencodeMode hint)
{
	...
	//newstr = _strdup(str); //bcmoon
	newstr = qr_strdup(str);
//	int len = strlen(str);
//
//	newstr = malloc(len);
		...
		QRencodeMode hint, int casesensitive)
{
	...
	if(!casesensitive) {
		...
		Free((U8*)newstr, strlen(newstr), eHeapTemp);
	} else {
